...
// moved to ios 11.4 by wld  0n 3/9/19
...
// MARK: - Navigation
    ...
    if (segue.identifier == "ShowReaders") {
        ...
        deviceInfoViewController = segue.destination as? ABDDeviceInfoViewController // as! to as?
...
@IBAction func unwindToMain(segue: UIStoryboardSegue) {
    ...
    fileprivate func showInputAlert(title:String, message:String,tag:Int,hexString:String) {
//        // Modify the master key.
//        let alert = UIAlertView(title: title, message: message , delegate: self, cancelButtonTitle: "OK", otherButtonTitles: "")
//        alert.alertViewStyle = .plainTextInput
//        alert.tag = tag
//        let alertTextField: UITextField? = alert.textField(at: 0)
//        alertTextField?.text = hexString
//        alert.show()
//
//
        let ac = UIAlertController(title: title, message:message , preferredStyle: .alert)
        ac.addTextField { tf in tf.autocapitalizationType = .words ; tf.text = hexString }
        ac.addAction(UIAlertAction(title: "Cancel", style: .cancel) {_ in  })
        ac.addAction(UIAlertAction(title: "OK", style: .default) {_ in
            if let proposal = ac.textFields?[0].text {
                if !proposal.trimmingCharacters(in: .whitespaces).isEmpty {
                    ...
                    case 0:
                    // Master key.
                    masterKey = ABDHex.byteArray(fromHexString: proposal)!
                    if !(self.masterKey == masterKey) {
                        self.masterKey = masterKey
                        self.masterKeyLabel.text = ABDHex.hexString(fromByteArray: self.masterKey!)
                        // Save the master key.
                    ...
                    case 1:
                        // Command APDU.
                        commandApdu = ABDHex.byteArray(fromHexString: (proposal))
                        if !(self.commandApdu ==  commandApdu) {
                            self.commandApdu = commandApdu
                            self.commandApduLabel.text = ABDHex.hexString(fromByteArray: self.commandApdu!)
                            // Save the command APDU.
                            self.defaults?.set(commandApdu, forKey: "CommandApdu")
                            self.defaults?.synchronize()
                        }
                    case 2:
                        // Escape command.
                        escapeCommand = ABDHex.byteArray(fromHexString: proposal)
                        if !(self.escapeCommand == escapeCommand) {
                            self.escapeCommand = escapeCommand
                            self.escapeCommandLabel.text = ABDHex.hexString(fromByteArray: self.escapeCommand!)
                            // Save the escape command.
                            self.defaults?.set(commandApdu, forKey: "EscapeCommand")
                            self.defaults?.synchronize()
...
//    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if (cell?.reuseIdentifier == "MasterKey") {
            // Modify the master key.
            showInputAlert(title: "Master Key", message: "Enter the HEX string:",tag:0,hexString:ABDHex.hexString(fromByteArray: masterKey! as Data?) )
        }
        else if (cell?.reuseIdentifier == "CommandApdu") {
            // Modify the command APDU.
            showInputAlert(title: "Command APDU", message: "Enter the HEX string:",  tag:1, hexString:ABDHex.hexString(fromByteArray: commandApdu!))
        }
        else if (cell?.reuseIdentifier == "EscapeCommand") {
            // Modify the escape command.
            
            showInputAlert(title: "Escape Command", message: "Enter the HEX string:",tag:2,hexString:ABDHex.hexString(fromByteArray: escapeCommand!))
...
//            bluetoothReader!.authenticate(withMasterKey: masterKey!)
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if label != nil {
            ...
            #else
            let labelRect: CGRect? = label?.text?.boundingRect(with: CGSize(width: tableView.frame.size.width - 40.0, height: CGFloat(MAXFLOAT)), options: .usesLineFragmentOrigin, attributes: [.font: label!.font  ], context: nil) // wld made it label!.fornt
                // Set the row height to 44 if it is less than zero (iOS 8.0).
                if height < 0 {
                    height = 44
                }
